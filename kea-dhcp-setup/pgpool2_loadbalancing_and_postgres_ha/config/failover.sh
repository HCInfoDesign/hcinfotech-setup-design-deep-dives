...
# Debug Failover
exec 1> >(tee -ia /var/log/pgpool-failover-logs/failover.log)
exec 2> >(tee -ia /var/log/pgpool-failover-logs/failover.log >& 2)

exec {FD}>> /var/log/pgpool-failover-logs/failover.log
...
PGHOME=<directory of the postgresql binaries>
...
/usr/bin/ssh -T ${SSH_OPTIONS} ${POSTGRESQL_STARTUP_USER}@${NEW_MAIN_NODE_HOST} ls /tmp > /dev/null
...
/usr/bin/ssh -T ${SSH_OPTIONS} ${POSTGRESQL_STARTUP_USER}@${NEW_MAIN_NODE_HOST} ${PGHOME}/bin/pg_ctl -D ${NEW_MAIN_NODE_PGDATA} -w promote
...
## Create the DR replication slot
${PGHOME}/bin/psql -h ${NEW_MAIN_NODE_HOST} -p ${NEW_MAIN_NODE_PORT} postgres \
   -c "SELECT pg_create_physical_replication_slot('b100pggrf4');"  >/dev/null 2>&1
